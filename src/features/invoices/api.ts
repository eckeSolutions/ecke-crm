...
/** `invoices` joined with the client's display name — same embedded-resource-select shape as the time-tracking feature's `TimeEntryWithClient`, and for the same reason (`supabase-js` doesn't type embedded resources from `Database` on its own). */
