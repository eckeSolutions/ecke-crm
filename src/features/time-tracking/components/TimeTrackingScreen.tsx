...
export function TimeTrackingScreen() {
  ...
  const createInvoiceFromSelection = () => {
    ...
    setSelected([]);
    navigate(`/invoices/new?client=${clientId}&entries=${entryIds}`);
  };
        ...
        <>
          {selected.length > 0 && (
            <EckeSelectionBar
              ...
              className="time-tracking__selection"
            >
              {noneInvoiced && (
          ...
          {selected.length > 0 && !(noneInvoiced && singleClient) && (
            <p className="time-tracking__selection-hint">
              {!noneInvoiced
          ...
          <EckeTableCard surface="glass" className="time-tracking__table">
