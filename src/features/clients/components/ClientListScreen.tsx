...
export function ClientListScreen() {
  ...
  useEffect(() => {
    ...
    const onClick = (event: MouseEvent) => {
      ...
      if (client) navigate(`/clients/${client.id}`, { viewTransition: true });
    };
  ...
  return (
    <>
      <EckePageHeader pageTitle="Kunden">
        <EckeButton surface="glass" emphasis="primary" onClick={() => navigate("/clients/new")}>
          Neuer Kunde
        </EckeButton>
      </EckePageHeader>
            ...
            >
              {selected.length === 1 && (
                <EckeButton
                  surface="glass"
                  emphasis="secondary"
                  onClick={() => navigate(`/clients/${selectedClients[0]!.id}/edit`)}
                >
                  Bearbeiten
                </EckeButton>
              )}
              {/* clients' RLS restricts delete to admin (docs/DATABASE_SCHEMA.md
                  ...
                  this feature's ContactsSection. */}
              {isAdmin && (
