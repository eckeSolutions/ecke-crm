...
/** RLS already scopes this to the caller's own rows unless admin (docs/DATABASE_SCHEMA.md §6); there's no month picker, same as Zeiterfassung's `fetchTimeEntriesForCurrentMonth` and the old app's `LedgerCubit.load()`, which always used `DateTime.now()`. */
