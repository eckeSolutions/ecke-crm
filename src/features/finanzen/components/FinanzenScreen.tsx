...
export function FinanzenScreen() {
  ...
  const saldo = useMemo(() => balance(entries), [entries]);
  ...
  // Row click -> edit, same composedPath() pattern Kunden's/Zeiterfassung's/
  // Rechnungen' tables use (ecke-table emits no row event of its own).
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div className="finanzen__stats">
        ...
        <EckeStatCard label="Saldo (Monat)" value={formatEuro(saldo)}>
      ...
      {entriesQuery.isPending ? (
        ...
        <>
          {selected.length > 0 && (
            <EckeSelectionBar summary={`${selected.length} Buchungen ausgewählt`} onEckeClear={() => setSelected([])} className="finanzen__selection">
          ...
          <EckeTableCard surface="glass" className="finanzen__table">
