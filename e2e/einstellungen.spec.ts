 ...
 * `e2e/rechnungen.spec.ts`'s PDF test uses.
 */
...
test.describe("Einstellungen", () => {
  ...
  // parallel workers would race on it the same way Finanzen's from-scratch
  // ledger did.
  ...
  test("an employee is bounced off the URL; an admin sees the three cards with real data", async ({ browser }) => {
    ...
    await employeePage.goto("/einstellungen");
    ...
    await adminPage.goto("/einstellungen");
