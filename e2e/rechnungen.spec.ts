...
async function deleteConfirmed(page: Page) {
  ...
  await page.waitForURL(/\/rechnungen$/, { timeout: 10_000 });
}
...
test.describe("Rechnungen", () => {
  ...
  test("an employee sees only their own invoices, an admin sees more", async ({ browser }) => {
    ...
    await adminPage.goto("/rechnungen");
    ...
    await employeePage.goto("/rechnungen");
  ...
  test("creating a new invoice needs a client first, via a native <select> (design-system issue #9 workaround)", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/rechnungen\/neu\?client=[0-9a-f-]{36}$/);
  ...
  test("a draft can be built, saved, edited, and deleted", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/rechnungen\/[0-9a-f-]{36}$/, { timeout: 10_000 });
    const url = page.url();
    ...
    await expect(page).toHaveURL(/\/rechnungen$/, { timeout: 10_000 });
    await page.goto(url);
  ...
  test("the Zeiterfassung handoff seeds a line item from the query params", async ({ page }) => {
    await login(page, ADMIN);

    // Find one of the seeded admin's own uninvoiced entries directly —
    // this test exercises the RECEIVING end of the handoff
    // (InvoiceEditorScreen reading ?client=&entries=), not Zeiterfassung's
    // sending end, which zeiterfassung.spec.ts already covers.
