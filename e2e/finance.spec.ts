 ...
 * the other feature specs document in the README. Unlike time tracking/
 * invoices, the seed data has NO `ledger_entries` rows at all, so every
...
async function deleteConfirmed(page: Page) {
  ...
  // register — unlike Invoices' delete (which navigates, giving a URL
...
test.describe("Finance", () => {
  ...
  // seeded rows to fall back on (unlike Clients/TimeTracking/Invoices,
  ...
  test("a new expense and income entry update the three stat cards correctly", async ({ page }) => {
    ...
    const statsText = page.locator(".finance__stats");
  ...
  test("an admin sees an employee's booking; the employee does not see the admin's", async ({ browser }) => {
    ...
    await employeePage.goto("/finance");
